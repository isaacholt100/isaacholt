...
- Taking the limit means we purification of the Lindblad equation can be expressed as a continuous MPS.
...
- If $T$ is ergodic with fixed point $rho^*$, then the long time limit of applying the channel is $
...
- Frustration is what leads to interesting physics (e.g. phase transitions) in many-body systems: it is due to the fact that the Hamiltonian is a sum of local terms, some of which do not commute. If they did commute, then the ground state would simply be an eigenstate of each term.
...
- The diagram below shows the possible reduced density matrices of the ground state of an XXZ Hamiltonian. The big triangle is the set of all reduced density matrices. The diamond is the set of separable states. The blue line encloses the set of all reduced density matrices when the XXZ Hamiltonian acts on a 1D lattice. The red line encloses the set of all reduced density matrices when the XXZ Hamiltonian acts on a 2D lattice. As the dimension tends to infinity, there is more and more frustration of more neighbours, and the set of reduced density matrices approaches the separable states (the diamond). #figure(image("convex-set.png", width: 60%))
...
#example[
    ...
    $ We can use PEPOs (projected entangled pair operators) to represent the Kramers-Wannier transformation in 2D (the first item in the diagram is the Kramers-Wannier transformation as a PEPO) - $circle$ represents a GHZ tensor in the $X$ basis with 5 qubits, $bot$ represents a GHZ tensor in the $Z$ basis with 3 qubits:
    #figure(
