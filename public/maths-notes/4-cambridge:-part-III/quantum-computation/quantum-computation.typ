...
Amplitude amplification is an extension of the key insights in Grover's algorithm.
...
#remark[
    Note that since $Q$ is a rotation by angle $2 theta$, $Q^(m_"opt") ket(psi)$ lies within angle $plus.minus theta$ of $ket(g)$, hence the $ket(g)$ component of $Q^(m_"opt") ket(psi)$ has amplitude $>= cos(theta)^2$. So for small $theta$, $
...
#algorithm("HHL")[
    We are given $ket(b) = 1/norm(b)_2 sum_(i = 0)^(N - 1) b_i ket(i)$ and a Hermitian matrx $A in CC^(N times N)$. Let $n = log N$.
...
#proofsketch[
    ...
    $ where $theta = arcsin(c \/ lambda)$ and $c <= min{abs(lambda_j): j in [N]}$, and e.g. $Ctrl("Rot") ket(y) ket(0) = ket(y) ket(0)$ for all other $y$ (this part is not important and could be chosen to be something else which is consistent with $Ctrl("Rot")$ being unitary). By linearity, this fully determines $Ctrl("Rot")$. \ By the above bounds on the $abs(lambda_j)$, we can choose $c = 1 \/ kappa$. So in $Ctrl("Rot")$, the angle depends on the first register but not on $A$ or $b$ (so we're not sneaking extra info in here). $Ctrl("Rot")$ can be implemented efficiently using $O(poly(n))$ one and two-qubit gates (by e.g. @thm:solovay-kitaev).
...
#definition[
    The *$1$-qubit Pauli group* is the subgroup of $U(2)$ defined as $
        cal(P)_1 := {plus.minus i, plus.minus 1} dot {I, X, Y, Z}
    $ with the usual rules $X Y = i Z$, etc. The *$n$-qubit Pauli group* is the finite subgroup of $U(2^n)$ defined as $
        cal(P)_n := cal(P)_1 tp dots.c tp cal(P)_1 = {P_1 tp cdots tp P_n: P_i in cal(P)_1 med forall i}.
    $
]<def:pauli-group>
#definition[
...
#example[
    ...
    $ is a Clifford circuit. More generally, the family of *cat/GHZ* circuits are Clifford circuits: $
        ket(0)^(tp n) -->^H ket(plus) ket(0)^(tp n - 1) -->^Ctrl(X_(1 2)) dots.c -->^Ctrl(X_(1 n)) 1/sqrt(2) (ket(0)^(tp n) + ket(1)^(tp n)).
    $
]
...
#proof[
    ...
    Noting that $Z = ket(0) bra(0) - ket(1) bra(1)$, write $Z_1 = Pi_0 - Pi_1$, where $Pi_0$ and $Pi_1$ are projectors onto the subspaces where first qubit is $0$ and $1$ respectively. Let the unitary part of $C$ be $C_M dots.c C_1$. Denoting by $p_i$ the probability of the measurement yielding outcome $i$ for $i = 0, 1$, we have $
