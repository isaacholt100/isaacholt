...
#algorithm(name: "Quantum Period Finding")[
    ...
    + Measure $QFT ket("per")$, yielding outcome $c = k_0 M\/r$ for some $0 <= k_0 < r$. So $c/M = k_0/r$. If $k_0$ is corpime to $r$, then the denominator of the simplified fraction $c/M$ is equal to $r$.
    + By the coprimality theorem, the probability that $k_0$ is coprime to $r$ is $O(1\/log log r)$.
    + To check if the computed value $r_0$ of $r$ is correct, compute/query $U_f$ to check if $f(0) = f(r_0)$ (this works since $f$ is periodic and one-to-one in each period).
...
#remark[
    ...
    $ For each $x_0 in ZZ\/M$, define the shift operator $k -> x_0 + k$ and the associated linear map $U(x_0): H_M -> H_M$, $ket(k) |-> ket(x_0 + k)$. Since $(ZZ\/M, +)$ is abelian, all $U(x_i)$ commute: $U(x_1) U(x_2) = U(x_1 + x_2) = U(x_2) U(x_1)$. Hence, they have a simultaneous basis of eigenvectors ${ket(chi_k): k in ZZ\/M}$, i.e. for all $k, x_0 in ZZ\/M$, $U(x_0) ket(chi_k) = w(x_0, k) ket(chi_k)$, where $abs(w(x_0, k)) = 1$. The ${chi_k}$ are called *shift-invariant states* and form an orthonormal basis for $H_M$.
