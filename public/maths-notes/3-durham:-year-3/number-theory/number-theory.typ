...
#definition[
    Let $L\/F$ field extension, $alpha in L$. The *field generated by $alpha$ over $F$* is the smallest subfield of $L$ containing $F$ and $alpha$: $ F(alpha) := inter.big_(K "field", \ F subset.eq K subset.eq L, \ alpha in K) K $ Generally, $F(alpha_1, ..., alpha_n)$ is smallest field extension of $F$ containing $alpha_1, ..., alpha_n$.
]
...
#proposition[
    ...
    - In any ring, the largest ideal contained in both $I$ and $J$ is $I inter J$.
]
#proposition[
    If $I$ and $J$ are non-zero ideals in $cal(O)_K$ then $ gcd(I, J) = I + J, quad "lcm"(I, J) = I inter J $
]
...
#lemma[
    For every non-zero ideal $I$ of $cal(O)_K$, $N(I) in I$, hence $I inter ZZ != ideal(0)$.
...
#definition[
    If $p in ZZ$ prime, and $ideal(p)_(O_K) = P_1^(e_1) dots.h.c P_r^(e_r)$ then $P_1, ..., P_r$ are the prime ideals *lying above* $p$. Equivalently, $P$ *lies above* $p$ if $P inter ZZ = ideal(p)_ZZ$.
]
#remark[
    If $P subset cal(O)_K$ nonzero prime ideal, then $N(P) in P inter ZZ$ so $P inter ZZ != ideal(0)$. But $P inter ZZ$ is prime ideal of $ZZ$ so $P inter ZZ = ideal(p)_ZZ$ for some prime $p in ZZ$. Hence $p in P$, $ideal(p)_(cal(O)_K) subset.eq P$ so $P | ideal(p)_(cal(O)_K)$. Hence every $P$ lies over some prime $p$.
]
#lemma[
...
#example[
    ...
    - But then $x = alpha + overline(alpha) in P inter ZZ = ideal(31)_ZZ$, but $31 divides.not x$, so we have a contradiction. So $ideal(alpha)$, $ideal(overline(alpha))$ are coprime ideals.
