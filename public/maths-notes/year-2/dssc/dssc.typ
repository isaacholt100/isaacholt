...
- Let $X_1, ..., X_n$ be random sample from distribution with cdf $F$. Then $ sup_(x in RR)|hat(F)(x) - F(x)| -> 0 quad "as" n -> oo $
...
- So $hat("Var")(S(underline(x)) -> (n - 1)/n s^2 / n$ as $B -> oo$.
...
- *Normal confidence interval for bootstrap estimate*: $ hat(theta) plus.minus z_(alpha \/ 2) sqrt(hat("Var")(S(underline(x)))) $ where $z_(alpha \/ 2)$ is $100(alpha \/ 2)%$ percentile of standard normal distribution. *Note*: only valid if size of data large enough, need to check for normality of bootstrap samples using quantile plot.
...
- To estimate $integral_a^b f(x) dif x$, use $X ~ "Unif"(a, b)$ $ mu = integral_a^b f(x) dif x = integral_a^b (b - a) f(x) 1/(b - a) = integral_a^b (b - a) f(x) f_X(x) = EE[(b - a) f(X)] $ which can be estimated by $ hat(mu)_n = (b - a) 1/n sum_(i = 1)^n g(X_i) $ for i.i.d. samples $X_i$.
