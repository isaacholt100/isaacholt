...
- Every group $(G, dot.op)$ isomorphic to a subgroup of $(S_G, compose)$ where $S_G$ is set of bijections of $G$ by isomorphism $psi(g) = L_g$, where $L_g(h) = g h$.
...
- Let $G$ act on $X$. Then $forall x in X$, exists bijection $ beta: cal(O)(x) -> {"left cosets of" "Stab"_G(x) "in" G} \ beta(g(x)) = g "Stab"_G(x) $
...
- If $x in cal(O)(y)$, then $"Stab"_G(x)$ and $"Stab"_G(y)$ conjugate to each other: $ exists h in H, quad "Stab"_G(x) = h "Stab"_G(y) h^(-1) $
