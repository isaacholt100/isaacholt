...
- *Definition*: *field homomorphism* is ring homomorphism $phi: K -> L$ between fields:
    - $phi(a + b) = phi(a) + phi(b)$
    - $phi(a b) = phi(a) phi(b)$
    - $phi(1) = 1$
    These imply $phi(0) = 0$, $phi(-a) = -phi(a)$, $phi(a^(-1)) = phi(a)^(-1)$.
- *Lemma*: let $phi: K -> L$ field homomorphism.
...
- *Lemma*: for any field $K$, $char(K)$ is either $0$ or prime.
- *Theorem*:
...
- *Remark*: let $char(K) = p$, then $p | binom(p, i)$ so $(a + b)^p = a^p + b^p$ in $K$. Also in $K[x]$ for $p > 2$ prime, $x^p - 1 = (x - 1)^p$.
- *Fermat's little theorem*: $forall a in FF_p, a^p = a$.

== Polynomials over fields

- *Definition*: *degree* of $f(x) = a_0 + a_1 x + dots.h.c + a_n x_n$, $a_n != 0$ is $deg(f(x)) = n$.
...
- *Rational root test*: if $f(x) = a_0 + dots.h.c + a_n x^n in ZZ[x]$ has rational root $b/c in QQ$ with $gcd(b, c) = 1$ then $b | a_0$ and $c | a_n$. *Note*: this can't be used to show $f$ is irreducible for $deg(f(x)) >= 4$.
- *Gauss's lemma*: let $f(x) in ZZ[x]$, $f(x) = g(x) h(x)$, $g(x), h(x) in QQ[x]$. Then $exists r in QQ: r g(x), r^(-1) h(x) in ZZ[x]$. i.e. if $f(x)$ can be factored in $QQ[x]$ it can be factored in $ZZ[x]$.
...
- *Example*: *$p$-th cyclotomic polynomial* is $ f(x) = (x^p - 1)/(x - 1) = 1 + dots.h.c + x^(p - 1) $ Now $ f(x + 1) = ((1 + x)^p - 1)/(1 + x - 1) = x^(p - 1) + p x^(p - 2) + dots.h.c + binom(p, p - 2) x + p $ so can apply Eisenstein with $p = p$.
- *Generalised Eisenstein's criterion*: let $R$ be integral domain, $K = "Frac"(R)$, $ f(x) = a_0 + dots.h.c + a_n x^n in R[x] $ If there is irreducible $p in R$ with $ p | a_0, ..., p | a_(n - 1), p divides.not a_n, p^2 divides.not a_0 $ then $f(x)$ is irreducible in $K[x]$.
...
- *Proposition*: let $theta$ transcendental over $K$, then exists unique $K$-isomorphism $phi: K(t) -> K(theta)$ such that $phi(t) = theta$: $ phi(f(t)/g(t)) = phi(f(theta)/g(theta)) $
...
- *Definition*: *degree* of field extension $L\/K$ is $ [L: K] := dim_L (F) $
- *Example*:
...
- *Proposition*: let $[L: K] < oo$, then $L\/K$ is algebraic extension and $L = K(alpha_1, ..., alpha_n)$ for some $alpha_1, ..., alpha_n in L$.
- *Tower law*: let $K subset.eq M subset.eq L$ *tower* of field extensions. Then
...
- *Lemma*: let $sigma: K -> K'$ isomorphism and $K(theta)\/K$, $theta$ has minimal polynomial $m(x) in K[x]$, $theta'$ be root of $sigma_* (m(x))$. Then there exists unique $K$-isomorphism $tau: K(theta) -> K'(theta')$ such that $tau(theta) = theta'$.
...
- *Definition*: $Aut(L\/K)$ is group of $K$-automorphisms of $L\/K$ with composition as the group operation.
- *Example*:
...
- *Theorem*: let $L = K(theta)$, $theta$ root of irreducible $f(x) in K[x]$, $deg(f) = n$. Then $|Aut(L\/K)| <= n$, with equality iff $f(x)$ has $n$ distinct roots in $L$.
...
- *Example*: let $K = FF_p (t)$, then $f(x) = x^p - t in K[x]$ is irreducible by Eisenstein's criterion with $p = t$, and $f(x) = x^p - theta^p = (x - theta)^p$ so $theta$ is root of multiplicity $p >= 2$. So $FF_p (theta)\/FF_p (t)$ is normal but not separable.
...
- *Example - biquadratic extension*: $L = QQ\(sqrt(2), sqrt(3)\)$ over $QQ$ is normal (as splitting field of $(x^2 - 2)(x^2 - 3)$ over $QQ$) and separable (as $char(QQ) = 0$), so is Galois extension. Let $sigma$ be given as before.
...
- *Remark*: it is not generally true that $\[K\(sqrt(a), sqrt(b)\): K\] = 4$, e.g. $QQ\(sqrt(2), sqrt(8)\) = QQ\(sqrt(2)\)$.
...
- *Definition*: for $f(x) in K[x]$, $deg(f) = n >= 1$, with $n$ distinct roots, the *Galois group* of $f(x)$, $G_f$, is Galois group of splitting field of $f(x)$ over $K$ (provided it is separable).
...
- *Definition*: if $L\/K$ is Galois, $Gal(L\/K) tilde.equiv ZZ\/n$, then $L$ is *cyclic extension* of $K$ of degree $n$.
...
- *Corollary*: let $K$ field, $n in NN$ be largest such that $K$ contains primitive $n$-th root of unity $zeta$. Then $mu(K)$ is cyclic subgroup in $K^*$ generated by $zeta$.

== $n$-th cyclotomic field extensions

- *Notation*: let $zeta_n = exp(2pi i \/ n) in CC$.
- *Definition*: $QQ\(zeta_n\)\/QQ$ is *$n$-th cyclotomic field extension*.
- *Proposition*: $QQ\(zeta_n\)\/QQ$ is Galois.
- *Definition*: $Phi_n (x) := product_(a in A) (x - zeta_n^a)$ where $A = {a in NN: 0 < a < n, gcd(a, n) = 1}$.
- *Proposition*: $Phi_n (x) in QQ[x]$ is irreducible and so is minimal polynomial of a primitive $n$-th root of unity over $QQ$. In particular, $[QQ\(zeta_n\): QQ] = phi(n)$, where $phi(n) = |(ZZ\/n)^times|$ is Euler function.
- *Proposition*: properties of $phi$ function:
...
- *Proposition*: $forall n in NN, x^n - 1 = product_(n_1 divides n) Phi_(n_1)(x)$.
- *Example*:
    - $Phi_1 (x) = x - 1$.
...
- *Theorem*: $Gal(QQ\(zeta_n\)\/QQ) tilde.equiv (ZZ\/n)^times$.
- *Corollary*: $Gal(QQ\(zeta_n\)\/QQ)$ is abelian so every subgroup is normal, so any subfield of $QQ\(zeta_n\)$ is Galois over $QQ$.
- *Corollary*: for $p$ prime, $G = Gal(QQ\(zeta_p\)\/QQ) tilde.equiv (ZZ\/p)^times tilde.equiv ZZ\/(p - 1)$. In particular, for $d | (p - 1)$, $QQ\(zeta_p\)$ contains exactly one subfield of degree $d$ and there are no other subfields.
- *Remark*: for $d = 2$ in above corollary, $QQ\(zeta_p\)$ contains unique quadratic subfield $QQ\(sqrt(D_p)\)$. $D_p = p$ if $p equiv 1 mod 4$ and $D_p = -p$ if $p equiv 3 mod 4$.
- *Example*: $Gal(QQ\(zeta_n\)\/QQ)$ not always cyclic, e.g. $Gal(QQ(zeta_8)\/QQ) tilde.equiv ZZ\/2 times ZZ\/2$.
- *Proposition*:
